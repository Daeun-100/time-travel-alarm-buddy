    ...
    <div className="space-y-6">
      ...
      <div className="flex space-x-2 border-b border-gray-200">
        ...
        >
          ...
          <span>요일별</span>
          <Badge variant="secondary" className="ml-1">
            {Object.values(weekdayGroups).filter(group => group.length > 0).length}
          </Badge>
        </Button>
      </div>
            ...
            <SelectContent>
              ...
              {destinations.map(dest => (
                <SelectItem key={dest} value={dest}>
                  {dest} ({schedules.filter(s => s.destination === dest).length}개)
                </SelectItem>
              ))}
            </SelectContent>
          </Select>
      ...
      {activeFilter === 'weekday' && (
        <div className="flex items-center space-x-2">
          ...
          <Select value={selectedWeekday} onValueChange={(value: Weekday | 'all') => setSelectedWeekday(value)}>
            <SelectTrigger className="w-48">
              <SelectValue />
            </SelectTrigger>
            <SelectContent>
              <SelectItem value="all">전체 요일</SelectItem>
              {Object.entries(weekdayGroups).map(([weekday, schedules]) => (
                schedules.length > 0 && (
                  <SelectItem key={weekday} value={weekday}>
                    {WEEKDAY_LABELS[weekday as Weekday]} ({schedules.length}개)
                  </SelectItem>
                )
              ))}
            </SelectContent>
      ...
      {/* 그룹 액션 바 */}
      {activeFilter === 'destination' && selectedDestination !== 'all' && onToggleGroupActive && onDeleteGroup && (
        <GroupActionBar
      ...
      {activeFilter === 'weekday' && selectedWeekday !== 'all' && onToggleGroupActive && onDeleteGroup && (
        <GroupActionBar
